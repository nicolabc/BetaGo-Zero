...
class Network:
	...
	def forwardPass(self,board):
		...
		if(self.useNoise):
			featuresToSend = copy.deepcopy(self.features)
			for x in range(0,9):
				featuresToSend[0][0][x]+=np.random.normal(0,0.002,9)
				featuresToSend[0][1][x]+=np.random.normal(0,0.002,9)

			featuresToSend = torch.tensor(featuresToSend,dtype=torch.float,device=cuda0,requires_grad=False)
			return self.mynet.forward(featuresToSend)
		else:
			featuresToSend = torch.tensor(self.features,dtype=torch.float,device=cuda0,requires_grad=False)
			return self.mynet.forward(featuresToSend)
...
class TwoLayerNet3(nn.Module):
	def __init__(self):
		...
		outputdim = 82
		self.classifier = nn.Sequential(
            		nn.Linear(self.postConvDim,outputdim*8),
					nn.ReLU(),
            		nn.Linear(outputdim*8,outputdim*4),
					nn.ReLU(),
            		nn.Linear(outputdim*4,outputdim*2),
					nn.ReLU(),
		...
		for name, param in self.named_parameters():
			#print(list(param.data.size()))
			torch.nn.init.uniform_(param, a=-0.001, b=0.001)
